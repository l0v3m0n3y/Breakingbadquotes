#ifndef BREAKINGBADQUOTES_H
#define BREAKINGBADQUOTES_H
...
private:
    std::string api_base = "https://api.breakingbadquotes.xyz/v1";
    ...
    http_request create_request(const std::string& endpoint, const std::string& method, const std::string& data = "") {
        ...
        // Set headers
        request.headers().add(U("Host"), U("api.breakingbadquotes.xyz"));
...
public:
    ...
    pplx::task<json::value> random_quote() {
        return make_api_call("/quotes","GET");
    }

    pplx::task<json::value> random_quotes(int count) {
        return make_api_call("/quotes/" + std::to_string(count),"GET");
    }
};
